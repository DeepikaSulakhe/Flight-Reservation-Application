    ...
    },
    {
      ...
      cell: (row, index, column, id) => (
        <ButtonGroup variant="contained" aria-label="outlined primary button group">
            <Button
              startIcon = {<Edit></Edit>}
              onClick={(e) => handleEditClick(e,row,"edit")}
            >
            </Button>
            <Button
 ...
 // const [pending, setPending] = React.useState(true);
  ...
  const [dataTable, setTable] = useState("")
    ...
    const fetchFlightData = () => {
      ...
      const handleEditClick = (e,flightObj, actionType) => {
         e.preventDefault()
         console.log("handle edit");
         if(actionType == "edit"){
            AdminService.updateFlightsDetailsAsAdmin(1,flightObj.flightNumber,flightObj).then((ret)=>{
